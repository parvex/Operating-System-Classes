...
#define head rdy_head[USER_Q]
#define tail rdy_tail[USER_Q]
 ...
 */
  ...
  if (rdy_head[USER_Q] == NIL_PROC) 
	return; /* One or more user processes queued. */
  else
	{
        if ((rdy_head[USER_Q])->queue_nr == 2)
		{
			akt = rdy_head[USER_Q];
			if (akt->p_nextready && akt->p_nextready->queue_nr == 3) /*//sprawdzamy czy za 2 nie ma trojki bo wtedy trzeba ja wstawic przed nastepna dwojka lub trojka bo popsuje sie kolejnosc*/
			{
				for (akt = rdy_head[USER_Q]->p_nextready->p_nextready; akt->p_nextready && akt->p_nextready->queue_nr!= 2 && akt->p_nextready->queue_nr != 3; akt = akt->p_nextready);
			}
			else
				for (;akt && akt->queue_nr != 3; akt = akt->p_nextready);
			if (akt) /*//to wstawiwamy za znaleziona trojke lub przed dwojka albo trjka*/
			{
				temp = rdy_head[USER_Q]->p_nextready; /*//zapisuje nastepny head by go nie zgubic*/

				rdy_head[USER_Q]->p_nextready = akt->p_nextready;
				akt->p_nextready = rdy_head[USER_Q];
				if (!rdy_head[USER_Q]->p_nextready) rdy_tail[USER_Q] = rdy_head[USER_Q];
				rdy_head[USER_Q] = temp;
			}
			else /*//wstawiamy na koniec*/
			{
		...
		else if(rdy_head[USER_Q]->queue_nr == 3)
		{
		  rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q];
		  rdy_tail[USER_Q] = rdy_head[USER_Q];
		  rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
		...
		else /*//szeregowanie procesu z pierwszej kolejki*/
		{
			for (akt = rdy_head[USER_Q];(akt != NIL_PROC) && (akt->queue_nr != 2) && (akt->queue_nr != 3); akt = akt->p_nextready);

			if (akt != NIL_PROC)
			{
				temp = rdy_head[USER_Q]->p_nextready; /*//zapisuje nastepny rdy_head[USER_Q] by go nie zgubic*/

				if (akt->queue_nr == 2 && akt->p_nextready && akt->p_nextready->queue_nr == 3) /*//jesli za 2 jest 3 to wstawiamy za 3 aby wymusic wykonanie procesu z 3 kolejki*/
				{
					rdy_head[USER_Q]->p_nextready = akt->p_nextready->p_nextready;
					if (!rdy_head[USER_Q]->p_nextready) rdy_tail[USER_Q] = rdy_head[USER_Q];  /*//zmieniamy ogon jesli sortowany proces jest ostatni*/
					akt->p_nextready->p_nextready = rdy_head[USER_Q];
				}
				else /*//po prostu wstawiamy za znaleziony element, czyli 2 lub 3*/
				{
					rdy_head[USER_Q]->p_nextready = akt->p_nextready;
					akt->p_nextready = rdy_head[USER_Q];
					if (!rdy_head[USER_Q]->p_nextready) rdy_tail[USER_Q] = rdy_head[USER_Q];
				...
				rdy_head[USER_Q] = temp; /*// zmiana glowy na zapisana*/
			}
			else /*//wstawiamy na koniec*/
			{
				  rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q];
