...
FORWARD _PROTOTYPE( int do_getqueue, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_setqueue, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_GETQUEUE:	r = do_getqueue(&m);	break;
	    case SYS_SETQUEUE:	r = do_setqueue(&m);	break;
	    default:		r = E_BAD_FCN;
	}
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->queue_nr = rpp->queue_nr;		/* initial number of queue for scheduling */
...
/*
	...
	Funkcja zwracająca wskazanie na element w strukturze PROC o zadanym PID.
	Funkcja szuka wyłącznie procesów typu USER
	
	W przypadku powodzenia - zwraca wskaźnik na proces
	W przypadku braku procesu - zwraca pusty wskaźnik
*/
struct proc * find_user_proc_by_pid(int pid)
{
	int I;
...
/*
	PRIVATE int do_getlot(m_ptr);
	
	Funkcja zwraca aktualną liczbę losów przypisaną do procesu o zadanym PID
	W przypadku nie odnalezienia procesu zwraca EINVAL
*/
PRIVATE int do_getqueue(m_ptr)
message *m_ptr;
{
	struct proc * rp;
...
/*
	PRIVATE int do_setlot(m_ptr);

	Funkcja ustawia obecną liczbe losów dla procesu.
	Zwracana wartość:
	
	1 - liczba losów została prawidłowo zmieniona, bez konieczności sortowania listy procesóœ gotowych
	2 - liczba losów została prawidłowo zmieniona, ale zaszła kolejność sortowania listy procesów gotowych
	
	EINVAL:
		- proces nie został odnaleziony
		- zadana liczba losów jest mniejsza od 1 lub większa od 20
		
*/
PRIVATE int do_setqueue(m_ptr)
message * m_ptr;
{
	struct proc * rp;
	int new_queue = m_ptr->m2_l2;
	...
	if(new_queue < 1 || new_queue > 3)
		return EINVAL;
	/*
		Jeśli proces jest gotowy, to zmieni się jego pozycja na liście oraz liczba ticketów procesów gotowych. To może mało eleganckie wyjście, ale na pewno bardzo skuteczne i całkiem logiczne
	*/	
	if(is_proc_ready(rp))
	{
		lock_unready(rp);
		rp->queue_nr = new_queue;
		lock_ready(rp);	
		return 2;
	}
